...
{
    NSString *authurl = kSinaWeiboAppAuthURL_iPad;
    if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:authurl]])
    {
        return  YES;
    }
    else
    {
        authurl = kSinaWeiboAppAuthURL_iPhone;
        return [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:authurl]];
    }
}
/**
...
//        [self removeAuthData];
        ...
        if (!ssoLoggingIn)
        {
            ...
            [[SinaWeiboAuthorizeView alloc] initWithAuthParams:params
                                                      delegate:self];
            authorizeView.needObserveScreenOrientation = _needObserveScreenOrientation;
            [authorizeView show];
...
- (void)authorizeViewDidCancel:(SinaWeiboAuthorizeView *)authView
{
    ...
    _needObserveScreenOrientation = NO;
}
    ...
    if ([urlString hasPrefix:self.ssoCallbackScheme])
    {
        ...
        else
        {
            ...
            if ([SinaWeiboRequest getParamValueFromUrl:urlString paramName:@"sso_error_user_cancelled"] ||
                //我添加的,未登录新浪微博的情况
                [SinaWeiboRequest getParamValueFromUrl:urlString paramName:@"failed"])
            {
                if ([delegate respondsToSelector:@selector(sinaweiboLogInDidCancel:)])
                {
                    [delegate sinaweiboLogInDidCancel:self];
